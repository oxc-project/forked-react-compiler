...
Ported AnalyseFunctions from TypeScript to Rust. Added new crate oxc_react_compiler_analyse_functions.
...
2. **Analysis subagent**: A `general-purpose` subagent that investigates the actual test failures. Its prompt MUST include:
   ...
   - **Pipeline path**: `compiler/crates/oxc_react_compiler/src/entrypoint/pipeline.rs`
...
5. **Pipeline path**: `compiler/crates/oxc_react_compiler/src/entrypoint/pipeline.rs`
...
2. **Instructions**: Port the `<PassName>` pass from TypeScript to Rust. Follow these steps:
   ...
   d. Read the Rust pipeline at `compiler/crates/oxc_react_compiler/src/entrypoint/pipeline.rs` and existing crate structure
