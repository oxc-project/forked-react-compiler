...
|---|---|
| `src/HIR/` (excluding `BuildHIR.ts`, `HIRBuilder.ts`) | `forked_react_compiler_hir` |
| `src/HIR/BuildHIR.ts`, `src/HIR/HIRBuilder.ts` | `forked_react_compiler_lowering` |
| `src/Babel/`, `src/Entrypoint/` | `forked_react_compiler` |
| `src/CompilerError.ts` | `forked_react_compiler_diagnostics` |
| `src/ReactiveScopes/` | `forked_react_compiler_reactive_scopes` |
| `src/<Name>/` | `forked_react_compiler_<name>` (1:1, e.g., `src/Optimization/` -> `forked_react_compiler_optimization`) |

3. Check if the pass is already ported:
   - Check if the corresponding Rust file exists in the target crate
   - Check if `compiler/crates/forked_react_compiler/src/entrypoint/pipeline.rs` already calls it
...
4. **Rust pipeline**: `compiler/crates/forked_react_compiler/src/entrypoint/pipeline.rs`
5. **Rust HIR types**: Key type files in `compiler/crates/forked_react_compiler_hir/src/` (especially `hir.rs`, `environment.rs`)
6. **Rust reactive types**: For reactive passes, also read `compiler/crates/forked_react_compiler_hir/src/reactive_function.rs`
