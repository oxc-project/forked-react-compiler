...
pub fn compile(
    ...
    let result = forked_react_compiler::compile_program(ast, scope, opts);
...
- **Consistent with existing infrastructure**: The `forked_react_compiler_ast` crate already handles JSON serde with all 1714 test fixtures passing
...
All integrations serialize to the same Babel JSON AST format that the `forked_react_compiler_ast` crate expects. This means:
