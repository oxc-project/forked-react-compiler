...
### `compiler/crates/oxc_react_compiler/src/bin/test_rust_port.rs`
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let ast: oxc_react_compiler_ast::File = serde_json::from_str(&ast_json)?;
    let scope: oxc_react_compiler_ast::ScopeInfo = serde_json::from_str(&scope_json)?;
...
**Crate structure**: The test binary lives in whatever crate contains the compiler pipeline (likely `oxc_react_compiler` or similar — to be created as passes are ported). It depends on `oxc_react_compiler_ast` for the input types.
...
- **Rust binary**: Takes pre-parsed AST JSON + Scope JSON (produced by the step 1 infrastructure). Deserializes into `oxc_react_compiler_ast::File` and `ScopeInfo`, then calls a Rust `lower()` that works with these types directly — no Babel dependency.
...
1. **Create the Rust compiler crate** — `compiler/crates/oxc_react_compiler/` with the binary target `test-rust-port`. Depends on `oxc_react_compiler_ast` for input types.
