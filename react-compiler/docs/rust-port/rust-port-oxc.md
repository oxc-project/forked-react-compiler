...
forked_react_compiler = { path = "../forked_react_compiler" }
...
Key files:
- Target types: `compiler/crates/forked_react_compiler_ast/src/scope.rs`
- Reference impl: `compiler/packages/babel-plugin-react-compiler-rust/src/scope.ts`

### 3. `convert_ast.rs` — OXC AST → forked_react_compiler_ast::File
...
) -> forked_react_compiler_ast::File
```

**Approach:** Recursive conversion, one function per AST category (statement, expression, pattern, JSX, etc.). Data is copied out of OXC's arena into owned `forked_react_compiler_ast` types.
...
**Comments:** Map OXC `Comment { kind, span }` → `forked_react_compiler_ast::common::Comment` (CommentBlock/CommentLine with start/end/value).

Key files:
- Target types: `compiler/crates/forked_react_compiler_ast/src/` (all modules)

### 4. `convert_ast_reverse.rs` — forked_react_compiler_ast → OXC AST
...
- Maps each `forked_react_compiler_ast` type back to its OXC equivalent
- The `CompileResult::Success { ast, .. }` returns `ast: Option<serde_json::Value>` — first deserialize to `forked_react_compiler_ast::File`, then convert to OXC
...
- Cross-validate AST conversion: parse same source with both Babel and OXC, convert both to `forked_react_compiler_ast::File`, diff
...
4. **`cargo test -p forked_react_compiler_oxc`** — run all crate tests
