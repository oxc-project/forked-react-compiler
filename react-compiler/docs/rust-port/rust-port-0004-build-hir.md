...
Port `BuildHIR.ts` (~4555 lines) and `HIRBuilder.ts` (~955 lines) into Rust equivalents in `compiler/crates/oxc_react_compiler_lowering/`. This is the first major compiler pass — it converts a Babel AST + scope info into the HIR control-flow graph representation.
...
```toml
# oxc_react_compiler_lowering/Cargo.toml
[dependencies]
oxc_react_compiler_ast = { path = "../oxc_react_compiler_ast" }
oxc_react_compiler_hir = { path = "../oxc_react_compiler_hir" }
...
The TypeScript `lower()` takes a `NodePath<t.Function>` and uses Babel's traversal API (`path.get()`, `path.scope.getBinding()`, etc.) extensively. The Rust port works with deserialized `oxc_react_compiler_ast` structs and the `ScopeInfo` from step 2.
...
1. Create `compiler/crates/oxc_react_compiler_diagnostics/` with `CompilerDiagnostic`, `CompilerError`, `ErrorCategory`, `CompilerErrorDetail`, `CompilerSuggestionOperation`.

2. Create `compiler/crates/oxc_react_compiler_hir/` with core types:
...
3. Create `compiler/crates/oxc_react_compiler_lowering/` with:
