...
**Location**: New file `compiler/crates/oxc_react_compiler_hir/src/reactive.rs`, re-exported from `lib.rs`
...
All of these are already defined in `oxc_react_compiler_hir`:
...
- `SourceLocation` (from `oxc_react_compiler_diagnostics`)
...
## 2. New Crate: `oxc_react_compiler_reactive_scopes`

```
compiler/crates/oxc_react_compiler_reactive_scopes/
  Cargo.toml
  src/
    lib.rs
...
**Cargo.toml dependencies**: `oxc_react_compiler_hir`, `oxc_react_compiler_diagnostics`, `indexmap`

Add to workspace `Cargo.toml` members and as dependency of `oxc_react_compiler`.
...
`compiler/crates/oxc_react_compiler_reactive_scopes/src/print_reactive_function.rs`:

- Entry point: `pub fn debug_reactive_function(func: &ReactiveFunction, env: &Environment) -> String`
- Uses the `DebugPrinter` struct pattern from `compiler/crates/oxc_react_compiler/src/debug_print.rs`
- Must produce output identical to the TS `printDebugReactiveFunction`

### Shared Print Helpers

Extract these as `pub` from `compiler/crates/oxc_react_compiler/src/debug_print.rs` (currently private):
...
let debug = oxc_react_compiler_reactive_scopes::debug_reactive_function(&reactive_fn, &env);
context.log_debug(DebugLogEntry::new("BuildReactiveFunction", debug));

oxc_react_compiler_reactive_scopes::assert_well_formed_break_targets(&reactive_fn)?;
context.log_debug(DebugLogEntry::new("AssertWellFormedBreakTargets", "ok".to_string()));

oxc_react_compiler_reactive_scopes::prune_unused_labels(&mut reactive_fn);
let debug = oxc_react_compiler_reactive_scopes::debug_reactive_function(&reactive_fn, &env);
...
1. Create `reactive.rs` in `oxc_react_compiler_hir` with all types from Section 1
2. Create `oxc_react_compiler_reactive_scopes` crate skeleton with `Cargo.toml` and empty `lib.rs`
...
- **Target**: `compiler/crates/oxc_react_compiler_reactive_scopes/src/build_reactive_function.rs`
...
Remove "BLOCKED" status from #32. Add crate mapping: `src/ReactiveScopes/` -> `oxc_react_compiler_reactive_scopes`.
...
- **Step 1**: Add `src/ReactiveScopes/` -> `oxc_react_compiler_reactive_scopes` to the TS-to-Rust crate mapping table
...
| File | Action |
|------|--------|
| `compiler/crates/oxc_react_compiler_hir/src/reactive.rs` | Create: all reactive types |
| `compiler/crates/oxc_react_compiler_hir/src/lib.rs` | Edit: `pub mod reactive; pub use reactive::*;` |
| `compiler/crates/oxc_react_compiler_reactive_scopes/` | Create: new crate |
| `compiler/crates/Cargo.toml` (workspace) | Edit: add member |
| `compiler/crates/oxc_react_compiler/Cargo.toml` | Edit: add dependency |
| `compiler/crates/oxc_react_compiler/src/debug_print.rs` | Edit: extract shared helpers as `pub` |
| `compiler/crates/oxc_react_compiler/src/entrypoint/pipeline.rs` | Edit: wire reactive passes |
