...
pub struct Environment {
    ...
    // Uses u32 to avoid depending on forked_react_compiler_ast types.
...
impl Environment {
        ...
        for detail in old.details {
            let is_invariant = match &detail {
                forked_react_compiler_diagnostics::CompilerErrorOrDiagnostic::Diagnostic(d) => {
                    d.category == forked_react_compiler_diagnostics::ErrorCategory::Invariant
                }
    ...
    pub fn has_todo_errors(&self) -> bool {
        self.errors.details.iter().any(|d| match d {
            forked_react_compiler_diagnostics::CompilerErrorOrDiagnostic::Diagnostic(d) => {
                d.category == forked_react_compiler_diagnostics::ErrorCategory::Todo
            }
            forked_react_compiler_diagnostics::CompilerErrorOrDiagnostic::ErrorDetail(d) => {
                d.category == forked_react_compiler_diagnostics::ErrorCategory::Todo
            }
