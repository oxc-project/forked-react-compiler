...
use forked_react_compiler::entrypoint::compile_result::LoggerEvent;
use forked_react_compiler::entrypoint::plugin_options::PluginOptions;

/// Result of compiling a program via the OXC frontend.
pub struct TransformResult {
    /// The compiled program as a forked_react_compiler_ast File (None if no changes needed).
    pub file: Option<forked_react_compiler_ast::File>,
...
pub fn transform(
    ...
    // Convert OXC AST to forked_react_compiler_ast
    ...
    // Run the compiler
    let result =
        forked_react_compiler::entrypoint::program::compile_program(file, scope_info.clone(), options);

    let diagnostics = compile_result_to_diagnostics(&result);
    let (program_ast, events, renames) = match result {
        forked_react_compiler::entrypoint::compile_result::CompileResult::Success {
            ast,
            events,
            renames,
            ..
        } => (ast, events, renames),
        forked_react_compiler::entrypoint::compile_result::CompileResult::Error {
            events, ..
        } => (None, events, Vec::new()),
    };
...
/// Emit a forked_react_compiler_ast::File to a string via OXC codegen.
