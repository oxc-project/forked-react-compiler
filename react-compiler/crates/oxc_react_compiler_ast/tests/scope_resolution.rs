...
fn scope_info_round_trip() {
        ...
        })
    {
        ...
        let scope_info: oxc_react_compiler_ast::scope::ScopeInfo =
...
fn rename_identifiers(file: &mut oxc_react_compiler_ast::File, si: &ScopeInfo) {
    visit_program(&mut file.program, si);
}

fn visit_program(prog: &mut oxc_react_compiler_ast::Program, si: &ScopeInfo) {
...
fn scope_resolution_rename() {
        ...
        })
    {
        ...
        let mut file: oxc_react_compiler_ast::File = match serde_json::from_str(&ast_json) {
            Ok(f) => f,
            Err(e) => {
