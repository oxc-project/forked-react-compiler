...
fn convert_loc(loc: &oxc_react_compiler_ast::common::SourceLocation) -> SourceLocation {
    SourceLocation {
        start: oxc_react_compiler_hir::Position {
            line: loc.start.line,
            column: loc.start.column,
            index: loc.start.index,
        },
        end: oxc_react_compiler_hir::Position {
            line: loc.end.line,
...
impl<'ast> Visitor<'ast> for IdentifierLocVisitor {
    ...
    fn enter_class_declaration(
        &mut self,
        node: &'ast oxc_react_compiler_ast::statements::ClassDeclaration,
        _scope_stack: &[ScopeId],
    ) {
        if let Some(id) = &node.id {
            self.insert_identifier(id, true);
    ...
    fn enter_class_expression(
        &mut self,
        node: &'ast oxc_react_compiler_ast::expressions::ClassExpression,
        _scope_stack: &[ScopeId],
    ) {
        if let Some(id) = &node.id {
            self.insert_identifier(id, true);
