...
use oxc_react_compiler_hir::dominator::post_dominator_frontier;
...
use oxc_react_compiler_utils::DisjointSet;
...
pub fn infer_reactive_places(
    ...
    let post_dominators = oxc_react_compiler_hir::dominator::compute_post_dominator_tree(
        func,
        env.next_block_id().0,
        false,
    )?;
...
fn is_reactive_controlled_block(
    block_id: BlockId,
    func: &HirFunction,
    post_dominators: &oxc_react_compiler_hir::dominator::PostDominator,
...
use oxc_react_compiler_hir::is_use_operator_type;
...
fn apply_reactive_flags_replay(
    ...
    for block_id in &block_ids {
        ...
        // 2b. Instructions
        let block = func.body.blocks.get(block_id).unwrap();
        let instr_ids: Vec<oxc_react_compiler_hir::InstructionId> = block.instructions.clone();
