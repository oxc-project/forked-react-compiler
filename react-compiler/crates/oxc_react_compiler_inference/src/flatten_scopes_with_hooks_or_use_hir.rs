...
use oxc_react_compiler_hir::{BlockId, HirFunction, InstructionValue, Terminal, Type};
...
fn is_hook_or_use(env: &Environment, ty: &Type) -> Result<bool, CompilerDiagnostic> {
    Ok(env.get_hook_kind_for_type(ty)?.is_some() || oxc_react_compiler_hir::is_use_operator_type(ty))
}
