...
use oxc_react_compiler_hir::{EvaluationOrder, HirFunction, IdentifierId, InstructionValue, ScopeId};
...
pub fn align_method_call_scopes(func: &mut HirFunction, env: &mut Environment) {
            ...
            match &instr.value {
                ...
                | InstructionValue::ObjectMethod { lowered_func, .. } => {
                    ...
                    let mut inner_func = std::mem::replace(
                        ...
                        oxc_react_compiler_ssa::enter_ssa::placeholder_function(),
                    );
    ...
    let original_range_ids: HashMap<ScopeId, oxc_react_compiler_hir::MutableRangeId> = range_updates
        .keys()
        .map(|&root_id| {
