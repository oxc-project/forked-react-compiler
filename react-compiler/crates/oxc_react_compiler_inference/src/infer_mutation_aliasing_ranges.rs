...
use oxc_react_compiler_diagnostics::{CompilerDiagnostic, ErrorCategory};
use oxc_react_compiler_hir::environment::Environment;
use oxc_react_compiler_hir::type_config::{ValueKind, ValueReason};
...
pub fn infer_mutation_aliasing_ranges(
    ...
    for &block_id in &block_order {
        ...
        if let oxc_react_compiler_hir::Terminal::Return { value, .. } = terminal {
        ...
        let terminal_effects = match terminal {
            oxc_react_compiler_hir::Terminal::MaybeThrow { effects, .. }
            | oxc_react_compiler_hir::Terminal::Return { effects, .. } => effects.clone(),
            _ => None,
        };
    ...
    for param in &mut func.params {
        let place = match param {
            oxc_react_compiler_hir::ParamPattern::Place(p) => p,
            oxc_react_compiler_hir::ParamPattern::Spread(s) => &mut s.place,
        };
    ...
    for &block_id in &block_order {
        ...
        match &mut block.terminal {
            oxc_react_compiler_hir::Terminal::Return { value, .. } => {
    ...
    for param in &func.params {
        let place = match param {
            oxc_react_compiler_hir::ParamPattern::Place(p) => p.clone(),
            oxc_react_compiler_hir::ParamPattern::Spread(s) => s.place.clone(),
        };
        tracked.push(place);
    }
    for ctx in &func.context {
