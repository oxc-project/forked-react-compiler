...
use forked_react_compiler_ast::Program;
use forked_react_compiler_ast::common::BaseNode;
use forked_react_compiler_ast::declarations::Declaration;
use forked_react_compiler_ast::declarations::ExportDefaultDecl;
use forked_react_compiler_ast::declarations::ExportDefaultDeclaration;
use forked_react_compiler_ast::declarations::ImportSpecifier;
use forked_react_compiler_ast::declarations::ModuleExportName;
...
use forked_react_compiler_ast::statements::*;
use forked_react_compiler_ast::visitor::AstWalker;
use forked_react_compiler_ast::visitor::MutVisitor;
use forked_react_compiler_ast::visitor::VisitResult;
use forked_react_compiler_ast::visitor::Visitor;
use forked_react_compiler_ast::visitor::walk_program_mut;
use forked_react_compiler_diagnostics::CompilerError;
use forked_react_compiler_diagnostics::CompilerErrorDetail;
use forked_react_compiler_diagnostics::CompilerErrorOrDiagnostic;
use forked_react_compiler_diagnostics::ErrorCategory;
use forked_react_compiler_diagnostics::SourceLocation;
...
use super::imports::get_forked_react_compiler_runtime_module;
...
struct CompileSource<'a> {
    ...
    fn_ast_loc: Option<forked_react_compiler_ast::common::SourceLocation>,
    fn_start: Option<u32>,
    fn_end: Option<u32>,
...
fn calls_hooks_or_creates_jsx_in_class_body(
    body: &forked_react_compiler_ast::expressions::ClassBody,
) -> bool {
    body.body
        .iter()
...
fn diagnostic_details_to_items(
    d: &forked_react_compiler_diagnostics::CompilerDiagnostic,
    filename: Option<&str>,
    ...
    let items: Vec<CompilerErrorItemInfo> = d
        ...
        .map(|item| match item {
            ...
            forked_react_compiler_diagnostics::CompilerDiagnosticDetail::Hint { message } => {
                CompilerErrorItemInfo {
                    kind: "hint".to_string(),
                    loc: None,
...
fn to_logger_loc(
    ast_loc: Option<&forked_react_compiler_ast::common::SourceLocation>,
    filename: Option<&str>,
...
fn suggestions_to_logger(
    suggestions: &Option<Vec<forked_react_compiler_diagnostics::CompilerSuggestion>>,
    ...
    suggestions.as_ref().map(|suggestions| {
        suggestions
            ...
            .map(|s| {
                let op = match s.op {
                    forked_react_compiler_diagnostics::CompilerSuggestionOperation::InsertBefore => {
                        LoggerSuggestionOp::InsertBefore
                    }
                    forked_react_compiler_diagnostics::CompilerSuggestionOperation::InsertAfter => {
                        LoggerSuggestionOp::InsertAfter
                    }
                    forked_react_compiler_diagnostics::CompilerSuggestionOperation::Remove => {
                        LoggerSuggestionOp::Remove
                    }
                    forked_react_compiler_diagnostics::CompilerSuggestionOperation::Replace => {
        ...
        if error_info.raw_message.is_none() {
            if let Some(ref source) = context.code {
                error_info.formatted_message = Some(
                    forked_react_compiler_diagnostics::code_frame::format_compiler_error(
                        err,
                        source,
                        source_fn.as_deref(),
                    ),
...
fn should_skip_compilation(program: &Program, options: &PluginOptions) -> bool {
    let runtime_module = get_forked_react_compiler_runtime_module(&options.target);
...
impl<'a, 'ast> Visitor<'ast> for FunctionDiscoveryVisitor<'a, 'ast> {
    ...
    fn enter_object_method(
        &mut self,
        _node: &'ast forked_react_compiler_ast::expressions::ObjectMethod,
        _scope_stack: &[ScopeId],
    ) {
        self.skip_body = false;
    }
        ...
        Statement::ExportNamedDeclaration(export) => {
            if let Some(ref decl) = export.declaration {
                ...
                export.specifiers.iter().any(|s| {
                    if let forked_react_compiler_ast::declarations::ExportSpecifier::ExportSpecifier(
                        spec,
                    ) = s
                    {
                        match &spec.local {
    ...
    for i in 0..num_params {
        ...
        if is_rest {
            new_params.push(PatternLike::RestElement(
                forked_react_compiler_ast::patterns::RestElement {
...
fn insert_after_fn_in_block(
    block: &mut forked_react_compiler_ast::statements::BlockStatement,
    node_id: u32,
    new_stmt: &Statement,
) -> bool {
    ...
    match expr {
        Expression::ObjectExpression(obj) => {
            for prop in &mut obj.properties {
                match prop {
                    forked_react_compiler_ast::expressions::ObjectExpressionProperty::ObjectMethod(m) => {
    ...
    let source_filename = program
        ...
        .or_else(|| {
            ...
            program.body.first().and_then(|stmt| {
                let base = match stmt {
                    forked_react_compiler_ast::statements::Statement::ExpressionStatement(s) => &s.base,
                    forked_react_compiler_ast::statements::Statement::VariableDeclaration(s) => &s.base,
                    forked_react_compiler_ast::statements::Statement::FunctionDeclaration(s) => &s.base,
                    _ => return None,
                };
...
mod tests {
    ...
    fn test_should_skip_compilation_no_import() {
        let program = Program {
            ...
            body: vec![],
            directives: vec![],
            source_type: forked_react_compiler_ast::SourceType::Module,
            interpreter: None,
            source_file: None,
        };
