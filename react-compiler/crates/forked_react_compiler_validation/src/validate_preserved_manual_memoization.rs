...
use forked_react_compiler_diagnostics::{
...
fn visit_terminal(
    terminal: &forked_react_compiler_hir::ReactiveTerminalStatement,
...
fn visit_pruned_scope(
    pruned: &forked_react_compiler_hir::PrunedReactiveScopeBlock,
    state: &mut VisitorState,
) {
...
fn destructure_lvalue_places(pattern: &forked_react_compiler_hir::Pattern) -> Vec<&Place> {
    let mut result = Vec::new();
    match pattern {
        ...
        forked_react_compiler_hir::Pattern::Object(obj) => {
            for entry in &obj.properties {
                match entry {
                    forked_react_compiler_hir::ObjectPropertyOrSpread::Property(prop) => {
                        result.push(&prop.place);
                    }
                    forked_react_compiler_hir::ObjectPropertyOrSpread::Spread(spread) => {
                        result.push(&spread.place);
                    }
                }
            }
    ...
    if is_subpath
        && (source.path.len() == inferred.path.len()
            || (inferred.path.len() >= source.path.len()
                && !inferred.path.iter().any(|t| t.property == forked_react_compiler_hir::PropertyLiteral::String("current".to_string()))))
    {
        CompareDependencyResult::Ok
    } else if is_subpath {
        if source.path.iter().any(|t| t.property == forked_react_compiler_hir::PropertyLiteral::String("current".to_string()))
            || inferred.path.iter().any(|t| t.property == forked_react_compiler_hir::PropertyLiteral::String("current".to_string()))
        {
...
fn pretty_print_scope_dependency(
    ...
    identifiers: &[forked_react_compiler_hir::Identifier],
) -> String {
    ...
    let path_str: String = dep_path.iter().map(|entry| {
        let prop = match &entry.property {
            forked_react_compiler_hir::PropertyLiteral::String(s) => s.clone(),
            forked_react_compiler_hir::PropertyLiteral::Number(n) => format!("{}", n),
        };
        if entry.optional {
            format!("?.{}", prop)
        } else {
    ...
    let normalized_dep = if let Some(temp) = temporaries.get(&dep_id) {
        ...
        ManualMemoDependency {
            root: ManualMemoDependencyRoot::NamedLocal {
                value: Place {
                    identifier: dep_id,
                    effect: forked_react_compiler_hir::Effect::Read,
                    reactive: false,
                    loc: ident.loc,
                },
                constant: false,
