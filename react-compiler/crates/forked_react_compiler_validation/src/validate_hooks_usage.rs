...
use forked_react_compiler_hir::visitors::{each_pattern_operand, each_terminal_operand};
use forked_react_compiler_hir::dominator::compute_unconditional_blocks;
use forked_react_compiler_hir::environment::{is_hook_name, Environment};
...
pub fn validate_hooks_usage(func: &HirFunction, env: &mut Environment) -> Result<(), forked_react_compiler_diagnostics::CompilerDiagnostic> {
        ...
        for &instr_id in &block.instructions {
            ...
            match &instr.value {
                ...
                InstructionValue::CallExpression { callee, args, .. } => {
                    ...
                    for arg in args {
                        let place = match arg {
                            forked_react_compiler_hir::PlaceOrSpread::Place(p) => p,
                            forked_react_compiler_hir::PlaceOrSpread::Spread(s) => &s.place,
                        };
                        visit_place(place, &value_kinds, &mut errors_by_loc, env)?;
                    }
                }
