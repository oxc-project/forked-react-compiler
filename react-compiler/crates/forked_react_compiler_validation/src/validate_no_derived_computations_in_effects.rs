...
fn record_phi_derivations(
    block: &forked_react_compiler_hir::BasicBlock,
...
fn record_instruction_derivations(
    ...
    match &instr.value {
        ...
        InstructionValue::CallExpression { callee, args, .. } => {
            ...
            {
                if let (
                    forked_react_compiler_hir::PlaceOrSpread::Place(arg0),
                    forked_react_compiler_hir::PlaceOrSpread::Place(arg1),
                ) = (&args[0], &args[1])
                {
            ...
            match &instr.value {
                InstructionValue::CallExpression { callee, args, .. } => {
                    ...
                    if is_set_state_type(callee_type)
                        && args.len() == 1
                    {
                        if let forked_react_compiler_hir::PlaceOrSpread::Place(arg0) = &args[0] {
                            let callee_metadata =
...
fn validate_effect_non_exp(
    ...
    for (_, block) in &effect_func.body.blocks {
        ...
        match &block.terminal {
            ...
            | forked_react_compiler_hir::Terminal::Throw { value, .. } => {
                if dep_values.contains_key(&value.identifier) {
                    return Vec::new();
                }
            }
            forked_react_compiler_hir::Terminal::If { test, .. }
            | forked_react_compiler_hir::Terminal::Branch { test, .. } => {
                if dep_values.contains_key(&test.identifier) {
                    return Vec::new();
                }
            }
            forked_react_compiler_hir::Terminal::Switch { test, .. } => {
                if dep_values.contains_key(&test.identifier) {
                    return Vec::new();
                }
            }
