...
use oxc_react_compiler_hir::EvaluationOrder;
use oxc_react_compiler_hir::FunctionId;
use oxc_react_compiler_hir::IdentifierName;
use oxc_react_compiler_hir::InstructionValue;
use oxc_react_compiler_hir::ParamPattern;
use oxc_react_compiler_hir::Place;
use oxc_react_compiler_hir::PrunedReactiveScopeBlock;
use oxc_react_compiler_hir::ReactiveBlock;
use oxc_react_compiler_hir::ReactiveFunction;
use oxc_react_compiler_hir::ReactiveScopeBlock;
use oxc_react_compiler_hir::ReactiveValue;
...
impl Scopes {
    ...
    fn visit_identifier(
        &mut self,
        identifier_id: oxc_react_compiler_hir::IdentifierId,
        env: &Environment,
    ) {
...
impl ReactiveFunctionVisitor for Visitor<'_> {
    ...
    fn visit_scope(&self, scope: &ReactiveScopeBlock, state: &mut Scopes) {
        ...
        let decl_ids: Vec<oxc_react_compiler_hir::IdentifierId> = scope_data
            .declarations
            .iter()
...
fn collect_globals_block(block: &ReactiveBlock, globals: &mut HashSet<String>, env: &Environment) {
    for stmt in block {
        match stmt {
            oxc_react_compiler_hir::ReactiveStatement::Instruction(instr) => {
                collect_globals_value(&instr.value, globals, env);
            }
            oxc_react_compiler_hir::ReactiveStatement::Scope(scope) => {
                collect_globals_block(&scope.instructions, globals, env);
            }
            oxc_react_compiler_hir::ReactiveStatement::PrunedScope(scope) => {
                collect_globals_block(&scope.instructions, globals, env);
            }
            oxc_react_compiler_hir::ReactiveStatement::Terminal(terminal) => {
...
fn collect_globals_terminal(
    stmt: &oxc_react_compiler_hir::ReactiveTerminalStatement,
    globals: &mut HashSet<String>,
    env: &Environment,
) {
    match &stmt.terminal {
        oxc_react_compiler_hir::ReactiveTerminal::Break { .. }
        | oxc_react_compiler_hir::ReactiveTerminal::Continue { .. } => {}
        oxc_react_compiler_hir::ReactiveTerminal::Return { .. }
        | oxc_react_compiler_hir::ReactiveTerminal::Throw { .. } => {}
        oxc_react_compiler_hir::ReactiveTerminal::For {
            init,
            test,
            update,
            loop_block,
        ...
        oxc_react_compiler_hir::ReactiveTerminal::ForOf {
            init,
            test,
            loop_block,
            ..
        ...
        oxc_react_compiler_hir::ReactiveTerminal::ForIn {
            init, loop_block, ..
        } => {
            collect_globals_value(init, globals, env);
            collect_globals_block(loop_block, globals, env);
        }
        oxc_react_compiler_hir::ReactiveTerminal::DoWhile {
            loop_block, test, ..
        } => {
            collect_globals_block(loop_block, globals, env);
            collect_globals_value(test, globals, env);
        }
        oxc_react_compiler_hir::ReactiveTerminal::While {
            test, loop_block, ..
        } => {
            collect_globals_value(test, globals, env);
            collect_globals_block(loop_block, globals, env);
        }
        oxc_react_compiler_hir::ReactiveTerminal::If {
            consequent,
            alternate,
            ..
        } => {
        ...
        oxc_react_compiler_hir::ReactiveTerminal::Switch { cases, .. } => {
            for case in cases {
        ...
        oxc_react_compiler_hir::ReactiveTerminal::Label { block, .. } => {
            collect_globals_block(block, globals, env);
        }
        oxc_react_compiler_hir::ReactiveTerminal::Try { block, handler, .. } => {
            collect_globals_block(block, globals, env);
