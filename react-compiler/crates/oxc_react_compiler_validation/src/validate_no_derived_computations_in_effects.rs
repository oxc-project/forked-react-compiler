...
fn record_phi_derivations(
    block: &oxc_react_compiler_hir::BasicBlock,
...
fn record_instruction_derivations(
    ...
    match &instr.value {
        ...
        InstructionValue::CallExpression { callee, args, .. } => {
            ...
            {
                if let (
                    oxc_react_compiler_hir::PlaceOrSpread::Place(arg0),
                    oxc_react_compiler_hir::PlaceOrSpread::Place(arg1),
                ) = (&args[0], &args[1])
                {
            ...
            match &instr.value {
                InstructionValue::CallExpression { callee, args, .. } => {
                    ...
                    if is_set_state_type(callee_type)
                        && args.len() == 1
                    {
                        if let oxc_react_compiler_hir::PlaceOrSpread::Place(arg0) = &args[0] {
                            let callee_metadata =
...
fn validate_effect_non_exp(
    ...
    for (_, block) in &effect_func.body.blocks {
        ...
        match &block.terminal {
            ...
            | oxc_react_compiler_hir::Terminal::Throw { value, .. } => {
                if dep_values.contains_key(&value.identifier) {
                    return Vec::new();
                }
            }
            oxc_react_compiler_hir::Terminal::If { test, .. }
            | oxc_react_compiler_hir::Terminal::Branch { test, .. } => {
                if dep_values.contains_key(&test.identifier) {
                    return Vec::new();
                }
            }
            oxc_react_compiler_hir::Terminal::Switch { test, .. } => {
                if dep_values.contains_key(&test.identifier) {
                    return Vec::new();
                }
            }
