...
use oxc_react_compiler_diagnostics::{
...
fn visit_terminal(
    terminal: &oxc_react_compiler_hir::ReactiveTerminalStatement,
    state: &mut VisitorState,
) {
    use oxc_react_compiler_hir::ReactiveTerminal;
...
fn visit_pruned_scope(
    pruned: &oxc_react_compiler_hir::PrunedReactiveScopeBlock,
    state: &mut VisitorState,
) {
...
fn destructure_lvalue_places(pattern: &oxc_react_compiler_hir::Pattern) -> Vec<&Place> {
    let mut result = Vec::new();
    match pattern {
        ...
        oxc_react_compiler_hir::Pattern::Object(obj) => {
            for entry in &obj.properties {
                match entry {
                    oxc_react_compiler_hir::ObjectPropertyOrSpread::Property(prop) => {
                        result.push(&prop.place);
                    }
                    oxc_react_compiler_hir::ObjectPropertyOrSpread::Spread(spread) => {
                        result.push(&spread.place);
                    }
                }
            }
    ...
    if is_subpath
        && (source.path.len() == inferred.path.len()
            || (inferred.path.len() >= source.path.len()
                && !inferred.path.iter().any(|t| t.property == oxc_react_compiler_hir::PropertyLiteral::String("current".to_string()))))
    {
        CompareDependencyResult::Ok
    } else if is_subpath {
        if source.path.iter().any(|t| t.property == oxc_react_compiler_hir::PropertyLiteral::String("current".to_string()))
            || inferred.path.iter().any(|t| t.property == oxc_react_compiler_hir::PropertyLiteral::String("current".to_string()))
        {
...
fn pretty_print_scope_dependency(
    ...
    identifiers: &[oxc_react_compiler_hir::Identifier],
) -> String {
    ...
    let path_str: String = dep_path.iter().map(|entry| {
        let prop = match &entry.property {
            oxc_react_compiler_hir::PropertyLiteral::String(s) => s.clone(),
            oxc_react_compiler_hir::PropertyLiteral::Number(n) => format!("{}", n),
        };
        if entry.optional {
            format!("?.{}", prop)
        } else {
    ...
    let normalized_dep = if let Some(temp) = temporaries.get(&dep_id) {
        ...
        ManualMemoDependency {
            root: ManualMemoDependencyRoot::NamedLocal {
                value: Place {
                    identifier: dep_id,
                    effect: oxc_react_compiler_hir::Effect::Read,
                    reactive: false,
                    loc: ident.loc,
                },
                constant: false,
