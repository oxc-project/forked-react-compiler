...
fn ref_type_of_type(
    ...
    if oxc_react_compiler_hir::is_ref_value_type(ty) {
        RefAccessType::RefValue {
            loc: None,
            ref_id: None,
        }
    } else if oxc_react_compiler_hir::is_use_ref_type(ty) {
        RefAccessType::Ref {
            ref_id: next_ref_id(),
        }
    } else {
...
fn is_ref_type(id: IdentifierId, identifiers: &[Identifier], types: &[Type]) -> bool {
    let identifier = &identifiers[id.0 as usize];
    oxc_react_compiler_hir::is_use_ref_type(&types[identifier.type_.0 as usize])
}

fn is_ref_value_type(id: IdentifierId, identifiers: &[Identifier], types: &[Type]) -> bool {
    let identifier = &identifiers[id.0 as usize];
    oxc_react_compiler_hir::is_ref_value_type(&types[identifier.type_.0 as usize])
}
...
fn validate_no_ref_access_in_render_impl(
    ...
    if ref_env.has_changed() {
        errors.push(
            CompilerDiagnostic::new(
                oxc_react_compiler_diagnostics::ErrorCategory::Invariant,
