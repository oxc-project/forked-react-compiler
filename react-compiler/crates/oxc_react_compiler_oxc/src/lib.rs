...
pub struct TransformResult {
    /// The compiled program as a oxc_react_compiler_ast File (None if no changes needed).
    pub file: Option<oxc_react_compiler_ast::File>,
...
pub fn transform(
    ...
    // Convert OXC AST to oxc_react_compiler_ast
    ...
    // Run the compiler
    let result =
        oxc_react_compiler::entrypoint::program::compile_program(file, scope_info.clone(), options);

    let diagnostics = compile_result_to_diagnostics(&result);
    let (program_ast, events, renames) = match result {
        oxc_react_compiler::entrypoint::compile_result::CompileResult::Success {
            ast,
            events,
            renames,
            ..
        } => (ast, events, renames),
        oxc_react_compiler::entrypoint::compile_result::CompileResult::Error {
            events, ..
        } => (None, events, Vec::new()),
    };
...
/// Emit a oxc_react_compiler_ast::File to a string via OXC codegen.
