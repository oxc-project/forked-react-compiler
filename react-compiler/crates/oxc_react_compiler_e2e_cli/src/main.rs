...
use clap::Parser;
use oxc_react_compiler::entrypoint::compile_result::LoggerEvent;
...
fn compile_swc(source: &str, filename: &str, mut options: PluginOptions) -> CompileOutput {
    ...
    let result = oxc_react_compiler_swc::transform(&module, source, options);
    ...
    let has_errors = result
        .diagnostics
        .iter()
        .any(|d| matches!(d.severity, oxc_react_compiler_swc::diagnostics::Severity::Error));

    match result.module {
        Some(compiled_module) => CompileOutput {
            code: Some(oxc_react_compiler_swc::emit(&compiled_module)),
            error: None,
            events,
        },
        None => {
            if has_errors {
                ...
                CompileOutput {
                    code: Some(oxc_react_compiler_swc::normalize_source(source)),
                    error: None,
                    events,
                }
            }
...
fn compile_oxc(
    ...
    if dump_scope {
        let scope_info =
            oxc_react_compiler_oxc::convert_scope::convert_scope_info(&semantic, &parsed.program);
        eprintln!("{}", serde_json::to_string_pretty(&scope_info).unwrap());
    }

    let result = oxc_react_compiler_oxc::transform(&parsed.program, &semantic, source, options);
    let events = result.events;
    ...
    match result.file {
        Some(ref file) => {
            ...
            CompileOutput {
                code: Some(oxc_react_compiler_oxc::emit(
                    file,
                    &emit_allocator,
                    Some(source),
                    &rename_plan,
