...
pub fn apply_gating_rewrites(
    program: &mut oxc_react_compiler_ast::Program,
            ...
            if !rewrite.is_export_default {
                ...
                if let Some(fn_name) = get_fn_decl_name_from_export_default(&original_stmt) {
                    ...
                    let re_export = Statement::ExportDefaultDeclaration(
                        oxc_react_compiler_ast::declarations::ExportDefaultDeclaration {
                            base: BaseNode::default(),
                            declaration: Box::new(
                                oxc_react_compiler_ast::declarations::ExportDefaultDecl::Expression(
    ...
    let original_fn = match &body[original_index] {
        ...
        Statement::ExportNamedDeclaration(end) => {
            if let Some(decl) = &end.declaration {
                if let oxc_react_compiler_ast::declarations::Declaration::FunctionDeclaration(fd) =
                    decl.as_ref()
                {
                    fd.clone()
                } else {
...
fn get_fn_decl_name_from_export_default(stmt: &Statement) -> Option<String> {
    match stmt {
        Statement::ExportDefaultDeclaration(ed) => match ed.declaration.as_ref() {
            oxc_react_compiler_ast::declarations::ExportDefaultDecl::FunctionDeclaration(fd) => {
...
fn extract_function_node_from_stmt(stmt: &Statement) -> Result<CompiledFunctionNode, CompilerDiagnostic> {
    match stmt {
        ...
        Statement::ExportDefaultDeclaration(ed) => match ed.declaration.as_ref() {
            ...
            oxc_react_compiler_ast::declarations::ExportDefaultDecl::Expression(expr) => {
                match expr.as_ref() {
                    Expression::ArrowFunctionExpression(arrow) => {
                        Ok(CompiledFunctionNode::ArrowFunctionExpression(arrow.clone()))
                    }
