...
use oxc_react_compiler_ast::Program;
use oxc_react_compiler_ast::common::BaseNode;
use oxc_react_compiler_ast::declarations::Declaration;
use oxc_react_compiler_ast::declarations::ExportDefaultDecl;
use oxc_react_compiler_ast::declarations::ExportDefaultDeclaration;
use oxc_react_compiler_ast::declarations::ImportSpecifier;
use oxc_react_compiler_ast::declarations::ModuleExportName;
...
use oxc_react_compiler_ast::statements::*;
use oxc_react_compiler_ast::visitor::AstWalker;
use oxc_react_compiler_ast::visitor::MutVisitor;
use oxc_react_compiler_ast::visitor::VisitResult;
use oxc_react_compiler_ast::visitor::Visitor;
use oxc_react_compiler_ast::visitor::walk_program_mut;
use oxc_react_compiler_diagnostics::CompilerError;
use oxc_react_compiler_diagnostics::CompilerErrorDetail;
use oxc_react_compiler_diagnostics::CompilerErrorOrDiagnostic;
use oxc_react_compiler_diagnostics::ErrorCategory;
use oxc_react_compiler_diagnostics::SourceLocation;
...
use super::imports::get_oxc_react_compiler_runtime_module;
...
struct CompileSource<'a> {
    ...
    fn_ast_loc: Option<oxc_react_compiler_ast::common::SourceLocation>,
    fn_start: Option<u32>,
    fn_end: Option<u32>,
...
fn calls_hooks_or_creates_jsx_in_class_body(
    body: &oxc_react_compiler_ast::expressions::ClassBody,
) -> bool {
    body.body
        .iter()
...
fn diagnostic_details_to_items(
    d: &oxc_react_compiler_diagnostics::CompilerDiagnostic,
    filename: Option<&str>,
    ...
    let items: Vec<CompilerErrorItemInfo> = d
        ...
        .map(|item| match item {
            ...
            oxc_react_compiler_diagnostics::CompilerDiagnosticDetail::Hint { message } => {
                CompilerErrorItemInfo {
                    kind: "hint".to_string(),
                    loc: None,
...
fn to_logger_loc(
    ast_loc: Option<&oxc_react_compiler_ast::common::SourceLocation>,
    filename: Option<&str>,
...
fn suggestions_to_logger(
    suggestions: &Option<Vec<oxc_react_compiler_diagnostics::CompilerSuggestion>>,
    ...
    suggestions.as_ref().map(|suggestions| {
        suggestions
            ...
            .map(|s| {
                let op = match s.op {
                    oxc_react_compiler_diagnostics::CompilerSuggestionOperation::InsertBefore => {
                        LoggerSuggestionOp::InsertBefore
                    }
                    oxc_react_compiler_diagnostics::CompilerSuggestionOperation::InsertAfter => {
                        LoggerSuggestionOp::InsertAfter
                    }
                    oxc_react_compiler_diagnostics::CompilerSuggestionOperation::Remove => {
                        LoggerSuggestionOp::Remove
                    }
                    oxc_react_compiler_diagnostics::CompilerSuggestionOperation::Replace => {
        ...
        if error_info.raw_message.is_none() {
            if let Some(ref source) = context.code {
                error_info.formatted_message = Some(
                    oxc_react_compiler_diagnostics::code_frame::format_compiler_error(
                        err,
                        source,
                        source_fn.as_deref(),
                    ),
...
fn should_skip_compilation(program: &Program, options: &PluginOptions) -> bool {
    let runtime_module = get_oxc_react_compiler_runtime_module(&options.target);
...
impl<'a, 'ast> Visitor<'ast> for FunctionDiscoveryVisitor<'a, 'ast> {
    ...
    fn enter_object_method(
        &mut self,
        _node: &'ast oxc_react_compiler_ast::expressions::ObjectMethod,
        _scope_stack: &[ScopeId],
    ) {
        self.skip_body = false;
    }
        ...
        Statement::ExportNamedDeclaration(export) => {
            if let Some(ref decl) = export.declaration {
                ...
                export.specifiers.iter().any(|s| {
                    if let oxc_react_compiler_ast::declarations::ExportSpecifier::ExportSpecifier(
                        spec,
                    ) = s
                    {
                        match &spec.local {
    ...
    for i in 0..num_params {
        ...
        if is_rest {
            new_params.push(PatternLike::RestElement(
                oxc_react_compiler_ast::patterns::RestElement {
...
fn insert_after_fn_in_block(
    block: &mut oxc_react_compiler_ast::statements::BlockStatement,
    node_id: u32,
    new_stmt: &Statement,
) -> bool {
    ...
    match expr {
        Expression::ObjectExpression(obj) => {
            for prop in &mut obj.properties {
                match prop {
                    oxc_react_compiler_ast::expressions::ObjectExpressionProperty::ObjectMethod(m) => {
    ...
    let source_filename = program
        ...
        .or_else(|| {
            ...
            program.body.first().and_then(|stmt| {
                let base = match stmt {
                    oxc_react_compiler_ast::statements::Statement::ExpressionStatement(s) => &s.base,
                    oxc_react_compiler_ast::statements::Statement::VariableDeclaration(s) => &s.base,
                    oxc_react_compiler_ast::statements::Statement::FunctionDeclaration(s) => &s.base,
                    _ => return None,
                };
...
mod tests {
    ...
    fn test_should_skip_compilation_no_import() {
        let program = Program {
            ...
            body: vec![],
            directives: vec![],
            source_type: oxc_react_compiler_ast::SourceType::Module,
            interpreter: None,
            source_file: None,
        };
