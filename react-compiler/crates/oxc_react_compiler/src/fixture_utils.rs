...
use oxc_react_compiler_ast::declarations::{Declaration, ExportDefaultDecl};
                            ...
                            Expression::FunctionExpression(func) => {
                                if index == function_index {
                                    let name = match &declarator.id {
                                        oxc_react_compiler_ast::patterns::PatternLike::Identifier(
                                            ident,
                                ...
                                if let Some(init) = &declarator.init {
                                    match init.as_ref() {
                                        Expression::FunctionExpression(func) => {
                                            if index == function_index {
                                                let name = match &declarator.id {
                                                    oxc_react_compiler_ast::patterns::PatternLike::Identifier(ident) => Some(ident.name.as_str()),
                                                    _ => func.id.as_ref().map(|id| id.name.as_str()),
                                                };
                                                return Some((
