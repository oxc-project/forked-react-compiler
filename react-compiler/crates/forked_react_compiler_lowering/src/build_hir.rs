...
use forked_react_compiler_ast::scope::BindingId;
use forked_react_compiler_ast::scope::BindingKind as AstBindingKind;
use forked_react_compiler_ast::scope::ScopeInfo;
use forked_react_compiler_ast::scope::ScopeId;
use forked_react_compiler_ast::scope::ScopeKind;
use forked_react_compiler_diagnostics::CompilerDiagnostic;
use forked_react_compiler_diagnostics::CompilerDiagnosticDetail;
use forked_react_compiler_diagnostics::CompilerError;
use forked_react_compiler_diagnostics::CompilerErrorDetail;
use forked_react_compiler_diagnostics::ErrorCategory;
use forked_react_compiler_hir::environment::Environment;
use forked_react_compiler_hir::*;
...
fn convert_loc(loc: &forked_react_compiler_ast::common::SourceLocation) -> SourceLocation {
    SourceLocation {
        start: Position {
            line: loc.start.line,
...
fn convert_opt_loc(
    loc: &Option<forked_react_compiler_ast::common::SourceLocation>,
) -> Option<SourceLocation> {
    loc.as_ref().map(convert_loc)
}

fn pattern_like_loc(
    pattern: &forked_react_compiler_ast::patterns::PatternLike,
) -> Option<forked_react_compiler_ast::common::SourceLocation> {
...
fn expression_loc(expr: &forked_react_compiler_ast::expressions::Expression) -> Option<SourceLocation> {
    use forked_react_compiler_ast::expressions::Expression;
    let loc = match expr {
...
fn expression_type_name(expr: &forked_react_compiler_ast::expressions::Expression) -> &'static str {
    use forked_react_compiler_ast::expressions::Expression;
    match expr {
...
fn lower_expression_to_temporary(
    builder: &mut HirBuilder,
    expr: &forked_react_compiler_ast::expressions::Expression,
) -> Result<Place, CompilerError> {
...
fn convert_binary_operator(op: &forked_react_compiler_ast::operators::BinaryOperator) -> BinaryOperator {
    use forked_react_compiler_ast::operators::BinaryOperator as AstOp;
    match op {
...
fn convert_unary_operator(op: &forked_react_compiler_ast::operators::UnaryOperator) -> UnaryOperator {
    use forked_react_compiler_ast::operators::UnaryOperator as AstOp;
    match op {
...
fn lower_arguments(
    builder: &mut HirBuilder,
    args: &[forked_react_compiler_ast::expressions::Expression],
...
fn convert_update_operator(op: &forked_react_compiler_ast::operators::UpdateOperator) -> UpdateOperator {
    match op {
        forked_react_compiler_ast::operators::UpdateOperator::Increment => UpdateOperator::Increment,
        forked_react_compiler_ast::operators::UpdateOperator::Decrement => UpdateOperator::Decrement,
    }
}
...
fn lower_member_expression(
    builder: &mut HirBuilder,
    member: &forked_react_compiler_ast::expressions::MemberExpression,
...
fn lower_member_expression_with_object(
    builder: &mut HirBuilder,
    member: &forked_react_compiler_ast::expressions::OptionalMemberExpression,
...
fn lower_expression(
    ...
    match expr {
        ...
        Expression::BinaryExpression(bin) => {
            ...
            if matches!(
                bin.operator,
                forked_react_compiler_ast::operators::BinaryOperator::Pipeline
            ) {
                builder.record_error(CompilerErrorDetail {
                    category: ErrorCategory::Todo,
        ...
        Expression::UnaryExpression(unary) => {
            ...
            match &unary.operator {
                forked_react_compiler_ast::operators::UnaryOperator::Delete => {
                ...
                forked_react_compiler_ast::operators::UnaryOperator::Throw => {
        ...
        Expression::LogicalExpression(expr) => {
            ...
            let hir_op = match expr.operator {
                forked_react_compiler_ast::operators::LogicalOperator::And => LogicalOperator::And,
                forked_react_compiler_ast::operators::LogicalOperator::Or => LogicalOperator::Or,
                forked_react_compiler_ast::operators::LogicalOperator::NullishCoalescing => {
            ...
            match update.argument.as_ref() {
                Expression::MemberExpression(member) => {
                    let binary_op = match &update.operator {
                        forked_react_compiler_ast::operators::UpdateOperator::Increment => {
                            BinaryOperator::Add
                        }
                        forked_react_compiler_ast::operators::UpdateOperator::Decrement => {
        ...
        Expression::AssignmentExpression(expr) => {
            use forked_react_compiler_ast::operators::AssignmentOperator;
            let loc = convert_opt_loc(&expr.base.loc);

            if matches!(expr.operator, AssignmentOperator::Assign) {
                // Simple `=` assignment
                match &*expr.left {
                    forked_react_compiler_ast::patterns::PatternLike::Identifier(ident) => {
                    ...
                    forked_react_compiler_ast::patterns::PatternLike::MemberExpression(member) => {
                        ...
                        let temp = if !member.computed
                            || matches!(
                                &*member.property,
                                forked_react_compiler_ast::expressions::Expression::NumericLiteral(_)
                            ) {
                            match &*member.property {
                                forked_react_compiler_ast::expressions::Expression::Identifier(
                                    prop_id,
                                ) => lower_value_to_temporary(
                                    builder,
                                    InstructionValue::PropertyStore {
                                ...
                                forked_react_compiler_ast::expressions::Expression::NumericLiteral(
                                    num,
                                ) => lower_value_to_temporary(
                                    builder,
                                    InstructionValue::PropertyStore {
                ...
                match &*expr.left {
                    forked_react_compiler_ast::patterns::PatternLike::Identifier(ident) => {
                        ...
                        let left_place = lower_expression_to_temporary(
                            builder,
                            &forked_react_compiler_ast::expressions::Expression::Identifier(ident.clone()),
                        )?;
                        let right = lower_expression_to_temporary(builder, &expr.right)?;
        ...
        Expression::ObjectExpression(obj) => {
            ...
            for prop in &obj.properties {
                match prop {
                    forked_react_compiler_ast::expressions::ObjectExpressionProperty::ObjectProperty(
                        p,
                    ) => {
                    ...
                    forked_react_compiler_ast::expressions::ObjectExpressionProperty::SpreadElement(
                        spread,
                    ) => {
                        let place = lower_expression_to_temporary(builder, &spread.argument)?;
                        properties.push(ObjectPropertyOrSpread::Spread(SpreadPattern { place }));
                    }
                    forked_react_compiler_ast::expressions::ObjectExpressionProperty::ObjectMethod(
                        method,
                    ) => {
        ...
        Expression::JSXElement(jsx_element) => {
            ...
            for attr_item in &jsx_element.opening_element.attributes {
                use forked_react_compiler_ast::jsx::JSXAttributeItem;
                use forked_react_compiler_ast::jsx::JSXAttributeName;
                use forked_react_compiler_ast::jsx::JSXAttributeValue;
                match attr_item {
                    ...
                    JSXAttributeItem::JSXAttribute(attr) => {
                        ...
                        let value = match &attr.value {
                            ...
                            Some(JSXAttributeValue::JSXExpressionContainer(container)) => {
                                use forked_react_compiler_ast::jsx::JSXExpressionContainerExpr;
                            ...
                            Some(JSXAttributeValue::JSXElement(el)) => {
                                let val = lower_expression(
                                    builder,
                                    &forked_react_compiler_ast::expressions::Expression::JSXElement(
                                        el.clone(),
                                    ),
                                )?;
                            ...
                            Some(JSXAttributeValue::JSXFragment(frag)) => {
                                let val = lower_expression(
                                    builder,
                                    &forked_react_compiler_ast::expressions::Expression::JSXFragment(
                                        frag.clone(),
                                    ),
                                )?;
                                lower_value_to_temporary(builder, val)?
            ...
            if is_fbt {
                ...
                if let forked_react_compiler_ast::jsx::JSXElementName::JSXIdentifier(jsx_id) =
                ...
                for (name, locations) in [
                    ...
                    if locations.len() > 1 {
                        ...
                        let details: Vec<CompilerDiagnosticDetail> = locations
                            ...
                            .collect();
                        let mut diag = forked_react_compiler_diagnostics::CompilerDiagnostic::new(
                            ErrorCategory::Todo,
...
fn is_binding_in_block_direct_statements(
    binding: &forked_react_compiler_ast::scope::BindingData,
    stmts: &[forked_react_compiler_ast::statements::Statement],
) -> bool {
...
#[allow(dead_code)]
fn pattern_declares_name(pattern: &forked_react_compiler_ast::patterns::PatternLike, name: &str) -> bool {
    use forked_react_compiler_ast::patterns::PatternLike;
    match pattern {
        PatternLike::Identifier(id) => id.name == name,
        PatternLike::ObjectPattern(op) => op.properties.iter().any(|prop| match prop {
            forked_react_compiler_ast::patterns::ObjectPatternProperty::ObjectProperty(p) => {
...
fn statement_start(stmt: &forked_react_compiler_ast::statements::Statement) -> Option<u32> {
    use forked_react_compiler_ast::statements::Statement;
    match stmt {
...
fn statement_end(stmt: &forked_react_compiler_ast::statements::Statement) -> Option<u32> {
    use forked_react_compiler_ast::statements::Statement;
    match stmt {
...
fn statement_loc(stmt: &forked_react_compiler_ast::statements::Statement) -> Option<SourceLocation> {
    use forked_react_compiler_ast::statements::Statement;
    let loc = match stmt {
...
fn collect_binding_names_from_pattern(
    pattern: &forked_react_compiler_ast::patterns::PatternLike,
    scope_id: forked_react_compiler_ast::scope::ScopeId,
    scope_info: &ScopeInfo,
...
fn lower_block_statement(
    builder: &mut HirBuilder,
    block: &forked_react_compiler_ast::statements::BlockStatement,
    parent_scope: Option<forked_react_compiler_ast::scope::ScopeId>,
) -> Result<(), CompilerError> {
...
fn lower_block_statement_with_scope(
    builder: &mut HirBuilder,
    block: &forked_react_compiler_ast::statements::BlockStatement,
    scope_override: forked_react_compiler_ast::scope::ScopeId,
) -> Result<(), CompilerError> {
...
fn lower_block_statement_inner(
    builder: &mut HirBuilder,
    block: &forked_react_compiler_ast::statements::BlockStatement,
    scope_override: Option<forked_react_compiler_ast::scope::ScopeId>,
    parent_scope: Option<forked_react_compiler_ast::scope::ScopeId>,
        ...
        for stmt in &block.body {
            if let Statement::VariableDeclaration(vd) = stmt {
                for d in &vd.declarations {
                    if let forked_react_compiler_ast::patterns::PatternLike::Identifier(id) = &d.id {
...
fn lower_statement(
    builder: &mut HirBuilder,
    stmt: &forked_react_compiler_ast::statements::Statement,
    label: Option<&str>,
    parent_scope: Option<forked_react_compiler_ast::scope::ScopeId>,
) -> Result<(), CompilerDiagnostic> {
    use forked_react_compiler_ast::statements::Statement;
    ...
    match stmt {
        ...
        Statement::VariableDeclaration(var_decl) => {
            use forked_react_compiler_ast::patterns::PatternLike;
            use forked_react_compiler_ast::statements::VariableDeclarationKind;
        ...
        Statement::ForStatement(for_stmt) => {
            ...
            let init_block = builder.try_enter(BlockKind::Loop, |builder, _block_id| {
                let init_loc = match &for_stmt.init {
                    ...
                    Some(init) => {
                        match init.as_ref() {
                            forked_react_compiler_ast::statements::ForInit::VariableDeclaration(var_decl) => {
                            ...
                            forked_react_compiler_ast::statements::ForInit::Expression(expr) => {
        ...
        Statement::ForInStatement(for_in) => {
            ...
            let left_loc = match for_in.left.as_ref() {
                forked_react_compiler_ast::statements::ForInOfLeft::VariableDeclaration(var_decl) => {
                    convert_opt_loc(&var_decl.base.loc).or(loc.clone())
                }
                forked_react_compiler_ast::statements::ForInOfLeft::Pattern(pat) => {
            ...
            let assign_result = match for_in.left.as_ref() {
                ...
                forked_react_compiler_ast::statements::ForInOfLeft::Pattern(pattern) => lower_assignment(
                    builder,
                    left_loc.clone(),
                    InstructionKind::Reassign,
                    pattern,
        ...
        Statement::TryStatement(try_stmt) => {
            ...
            let handler_binding_info: Option<(Place, forked_react_compiler_ast::patterns::PatternLike)> =
                if let Some(param) = &handler_clause.param {
                    ...
                    let is_destructuring = matches!(
                        param,
                        forked_react_compiler_ast::patterns::PatternLike::ObjectPattern(_)
                            | forked_react_compiler_ast::patterns::PatternLike::ArrayPattern(_)
                    );
                    if is_destructuring {
                        // Iterate the pattern to find all identifier locs for error reporting
                        fn collect_identifier_locs(
                            pat: &forked_react_compiler_ast::patterns::PatternLike,
                            locs: &mut Vec<Option<SourceLocation>>,
                        ) {
                            match pat {
                                forked_react_compiler_ast::patterns::PatternLike::Identifier(id) => {
                                    locs.push(convert_opt_loc(&id.base.loc));
                                }
                                forked_react_compiler_ast::patterns::PatternLike::ObjectPattern(obj) => {
                                    for prop in &obj.properties {
                                        match prop {
                                            forked_react_compiler_ast::patterns::ObjectPatternProperty::ObjectProperty(p) => {
                                ...
                                forked_react_compiler_ast::patterns::PatternLike::ArrayPattern(arr) => {
                                    for elem in &arr.elements {
                                        if let Some(e) = elem {
        ...
        Statement::TSEnumDeclaration(e) => {
            ...
            let original_node = serde_json::to_value(
                &forked_react_compiler_ast::statements::Statement::TSEnumDeclaration(e.clone()),
            )
            .ok();
            lower_value_to_temporary(
                builder,
        ...
        Statement::EnumDeclaration(e) => {
            let loc = convert_opt_loc(&e.base.loc);
            let original_node = serde_json::to_value(
                &forked_react_compiler_ast::statements::Statement::EnumDeclaration(e.clone()),
            )
            .ok();
            lower_value_to_temporary(
                builder,
...
enum FunctionBody<'a> {
    Block(&'a forked_react_compiler_ast::statements::BlockStatement),
    Expression(&'a forked_react_compiler_ast::expressions::Expression),
}
...
pub fn lower(
    ...
    let (params, body, generator, is_async, loc, start, end, ast_id) = match func {
        ...
        FunctionNode::ArrowFunctionExpression(arrow) => {
            let body = match arrow.body.as_ref() {
                forked_react_compiler_ast::expressions::ArrowFunctionBody::BlockStatement(block) => {
                    FunctionBody::Block(block)
                }
                forked_react_compiler_ast::expressions::ArrowFunctionBody::Expression(expr) => {
    ...
    let context_map: IndexMap<forked_react_compiler_ast::scope::BindingId, Option<SourceLocation>> =
        IndexMap::new();
...
fn lower_assignment(
    ...
    loc: Option<SourceLocation>,
    kind: InstructionKind,
    target: &forked_react_compiler_ast::patterns::PatternLike,
    value: Place,
    ...
    match target {
        ...
        PatternLike::MemberExpression(member) => {
            ...
            let temp = if !member.computed
                ...
                match &*member.property {
                    forked_react_compiler_ast::expressions::Expression::Identifier(prop_id) => {
                        lower_value_to_temporary(
                            builder,
                            InstructionValue::PropertyStore {
                                object,
                    ...
                    forked_react_compiler_ast::expressions::Expression::NumericLiteral(num) => {
                        lower_value_to_temporary(
                            builder,
                            InstructionValue::PropertyStore {
                                object,
                ...
                if matches!(
                    &*member.property,
                    forked_react_compiler_ast::expressions::Expression::PrivateName(_)
                ) {
                    builder.record_error(CompilerErrorDetail {
        ...
        PatternLike::ObjectPattern(pattern) => {
            ...
            let force_temporaries = if kind == InstructionKind::Reassign {
                use forked_react_compiler_ast::patterns::ObjectPatternProperty;
                let mut found = false;
            ...
            for prop in &pattern.properties {
                match prop {
                    forked_react_compiler_ast::patterns::ObjectPatternProperty::RestElement(rest) => {
                        match &*rest.argument {
                            PatternLike::Identifier(id) => {
                                let start = id.base.start.unwrap_or(0);
                                let is_context =
...
fn pattern_like_hir_loc(pat: &forked_react_compiler_ast::patterns::PatternLike) -> Option<SourceLocation> {
    convert_opt_loc(&pattern_like_loc(pat))
}

fn lower_optional_member_expression(
    builder: &mut HirBuilder,
    expr: &forked_react_compiler_ast::expressions::OptionalMemberExpression,
) -> Result<InstructionValue, CompilerError> {
...
fn lower_optional_call_expression(
    builder: &mut HirBuilder,
    expr: &forked_react_compiler_ast::expressions::OptionalCallExpression,
) -> Result<InstructionValue, CompilerError> {
...
fn lower_function(
    ...
    let function_scope =
        if let Some(scope) = builder.scope_info().resolve_scope_for_node(func_node_id) {
            ...
            let scope_info = builder.scope_info();
            let mapped: std::collections::HashSet<forked_react_compiler_ast::scope::ScopeId> =
                scope_info.node_id_to_scope.values().copied().collect();
            let param_names: Vec<String> = params
                .iter()
                .filter_map(|p| {
                    if let forked_react_compiler_ast::patterns::PatternLike::Identifier(id) = p {
                        Some(id.name.clone())
                    } else {
                        None
                    }
            ...
            while changed {
                ...
                for (i, scope) in scope_info.scopes.iter().enumerate() {
                    let sid = forked_react_compiler_ast::scope::ScopeId(i as u32);
    ...
    let merged_context: IndexMap<forked_react_compiler_ast::scope::BindingId, Option<SourceLocation>> = {
...
fn lower_function_declaration(
    builder: &mut HirBuilder,
    func_decl: &forked_react_compiler_ast::statements::FunctionDeclaration,
) -> Result<(), CompilerError> {
...
fn lower_function_for_object_method(
    builder: &mut HirBuilder,
    method: &forked_react_compiler_ast::expressions::ObjectMethod,
...
fn lower_inner(
    params: &[forked_react_compiler_ast::patterns::PatternLike],
    body: FunctionBody<'_>,
    id: Option<&str>,
    generator: bool,
    is_async: bool,
    ...
    scope_info: &ScopeInfo,
    env: &mut Environment,
    parent_bindings: Option<IndexMap<forked_react_compiler_ast::scope::BindingId, IdentifierId>>,
    parent_used_names: Option<IndexMap<String, forked_react_compiler_ast::scope::BindingId>>,
    context_map: IndexMap<forked_react_compiler_ast::scope::BindingId, Option<SourceLocation>>,
    function_scope: forked_react_compiler_ast::scope::ScopeId,
    component_scope: forked_react_compiler_ast::scope::ScopeId,
    context_identifiers: &HashSet<forked_react_compiler_ast::scope::BindingId>,
    is_top_level: bool,
    identifier_locs: &IdentifierLocIndex,
) -> Result<
    (
        HirFunction,
        IndexMap<String, forked_react_compiler_ast::scope::BindingId>,
        IndexMap<forked_react_compiler_ast::scope::BindingId, IdentifierId>,
    ),
    CompilerError,
> {
    ...
    for param in params {
        match param {
            ...
            forked_react_compiler_ast::patterns::PatternLike::RestElement(rest) => {
            ...
            forked_react_compiler_ast::patterns::PatternLike::ObjectPattern(_)
            | forked_react_compiler_ast::patterns::PatternLike::ArrayPattern(_)
            | forked_react_compiler_ast::patterns::PatternLike::AssignmentPattern(_) => {
                let param_loc = convert_opt_loc(&pattern_like_loc(param));
            ...
            forked_react_compiler_ast::patterns::PatternLike::TSAsExpression(_)
            | forked_react_compiler_ast::patterns::PatternLike::TSSatisfiesExpression(_)
            | forked_react_compiler_ast::patterns::PatternLike::TSNonNullExpression(_)
            | forked_react_compiler_ast::patterns::PatternLike::TSTypeAssertion(_) => {}
        }
    }
...
fn lower_jsx_element_name(
    builder: &mut HirBuilder,
    name: &forked_react_compiler_ast::jsx::JSXElementName,
) -> Result<JsxTag, CompilerError> {
    use forked_react_compiler_ast::jsx::JSXElementName;
    match name {
...
fn lower_jsx_member_expression(
    builder: &mut HirBuilder,
    expr: &forked_react_compiler_ast::jsx::JSXMemberExpression,
) -> Result<Place, CompilerError> {
    use forked_react_compiler_ast::jsx::JSXMemberExprObject;
...
fn lower_jsx_element(
    builder: &mut HirBuilder,
    child: &forked_react_compiler_ast::jsx::JSXChild,
) -> Result<Option<Place>, CompilerError> {
    use forked_react_compiler_ast::jsx::JSXChild;
    use forked_react_compiler_ast::jsx::JSXExpressionContainerExpr;
    match child {
        ...
        JSXChild::JSXElement(element) => {
            let value = lower_expression(
                builder,
                &forked_react_compiler_ast::expressions::Expression::JSXElement(element.clone()),
            )?;
        ...
        JSXChild::JSXFragment(fragment) => {
            let value = lower_expression(
                builder,
                &forked_react_compiler_ast::expressions::Expression::JSXFragment(fragment.clone()),
            )?;
            Ok(Some(lower_value_to_temporary(builder, value)?))
        }
...
fn lower_object_method(
    ...
    use forked_react_compiler_ast::expressions::ObjectMethodKind;
...
fn lower_object_property_key(
    builder: &mut HirBuilder,
    key: &forked_react_compiler_ast::expressions::Expression,
    computed: bool,
...
fn is_reorderable_expression(
    ...
    match expr {
        ...
        Expression::UnaryExpression(unary) => {
            use forked_react_compiler_ast::operators::UnaryOperator;
            matches!(
                unary.operator,
        ...
        Expression::ObjectExpression(obj) => obj.properties.iter().all(|prop| match prop {
            forked_react_compiler_ast::expressions::ObjectExpressionProperty::ObjectProperty(p) => {
        ...
        Expression::ArrowFunctionExpression(arrow) => {
            use forked_react_compiler_ast::expressions::ArrowFunctionBody;
...
fn gather_captured_context(
...
) -> IndexMap<forked_react_compiler_ast::scope::BindingId, Option<SourceLocation>> {
    ...
    let mut captured: std::collections::HashMap<
        forked_react_compiler_ast::scope::BindingId,
...
fn capture_scopes(
    scope_info: &ScopeInfo,
    from: forked_react_compiler_ast::scope::ScopeId,
    to: forked_react_compiler_ast::scope::ScopeId,
) -> IndexSet<forked_react_compiler_ast::scope::ScopeId> {
...
fn collect_fbt_sub_tags(
    children: &[forked_react_compiler_ast::jsx::JSXChild],
    tag_name: &str,
    ...
    for child in children {
        match child {
            ...
            JSXChild::JSXExpressionContainer(container) => {
                if let forked_react_compiler_ast::jsx::JSXExpressionContainerExpr::Expression(expr) =
...
fn collect_fbt_sub_tags_from_element(
    el: &forked_react_compiler_ast::jsx::JSXElement,
    tag_name: &str,
    enum_locs: &mut Vec<Option<SourceLocation>>,
    plural_locs: &mut Vec<Option<SourceLocation>>,
    pronoun_locs: &mut Vec<Option<SourceLocation>>,
    ...
    for attr in &el.opening_element.attributes {
        if let forked_react_compiler_ast::jsx::JSXAttributeItem::JSXAttribute(a) = attr {
            if let Some(val) = &a.value {
                if let forked_react_compiler_ast::jsx::JSXAttributeValue::JSXExpressionContainer(
                    container,
                ) = val
                {
                    if let forked_react_compiler_ast::jsx::JSXExpressionContainerExpr::Expression(expr) =
                ...
                } else if let forked_react_compiler_ast::jsx::JSXAttributeValue::JSXElement(nested) = val {
                    collect_fbt_sub_tags_from_element(
                        nested,
                        tag_name,
                        enum_locs,
...
fn collect_fbt_sub_tags_from_expr(
    ...
    match expr {
        ...
        Expression::ArrowFunctionExpression(arrow) => match arrow.body.as_ref() {
            forked_react_compiler_ast::expressions::ArrowFunctionBody::Expression(body_expr) => {
                collect_fbt_sub_tags_from_expr(
                    body_expr,
                    tag_name,
                    enum_locs,
...
fn collect_fbt_sub_tags_from_stmts(
    ...
    for stmt in stmts {
        if let forked_react_compiler_ast::statements::Statement::ReturnStatement(ret) = stmt {
            if let Some(arg) = &ret.argument {
                collect_fbt_sub_tags_from_expr(arg, tag_name, enum_locs, plural_locs, pronoun_locs);
            }
        } else if let forked_react_compiler_ast::statements::Statement::ExpressionStatement(expr_stmt) =
            stmt
        {
            collect_fbt_sub_tags_from_expr(
...
fn collect_identifier_node_ids_from_expr(
    ...
    match expr {
        ...
        Expression::ArrowFunctionExpression(arrow) => match arrow.body.as_ref() {
            ...
            forked_react_compiler_ast::expressions::ArrowFunctionBody::Expression(e) => {
                collect_identifier_node_ids_from_expr(e, positions);
            }
        },
        Expression::JSXElement(el) => {
            if let forked_react_compiler_ast::jsx::JSXElementName::JSXIdentifier(id) =
            ...
            for attr in &el.opening_element.attributes {
                match attr {
                        ...
                        if let Some(val) = &a.value {
                            match val {
                                forked_react_compiler_ast::jsx::JSXAttributeValue::JSXExpressionContainer(c) => {
                                    if let forked_react_compiler_ast::jsx::JSXExpressionContainerExpr::Expression(e) = &c.expression {
                                        collect_identifier_node_ids_from_expr(e, positions);
                                    }
                                }
                                _ => {}
                    ...
                    forked_react_compiler_ast::jsx::JSXAttributeItem::JSXSpreadAttribute(a) => {
            ...
            for child in &el.children {
                match child {
                    ...
                    forked_react_compiler_ast::jsx::JSXChild::JSXElement(child_el) => {
                        collect_identifier_node_ids_from_expr(
                            &Expression::JSXElement(child_el.clone()),
                            positions,
                        );
                    }
                    forked_react_compiler_ast::jsx::JSXChild::JSXSpreadChild(s) => {
                        collect_identifier_node_ids_from_expr(&s.expression, positions);
                    }
                    _ => {}
                }
        ...
        Expression::ObjectExpression(obj) => {
            for prop in &obj.properties {
                match prop {
                    ...
                    forked_react_compiler_ast::expressions::ObjectExpressionProperty::SpreadElement(s) => {
                        collect_identifier_node_ids_from_expr(&s.argument, positions);
                    }
                    _ => {}
                }
