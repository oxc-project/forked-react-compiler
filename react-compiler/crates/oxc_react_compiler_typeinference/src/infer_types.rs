...
use oxc_react_compiler_hir::environment::{Environment, is_hook_name};
use oxc_react_compiler_hir::object_shape::{
    ShapeRegistry,
...
use oxc_react_compiler_ssa::enter_ssa::placeholder_function;
...
fn apply_instruction_operands(
    ...
    match value {
        ...
        InstructionValue::CallExpression { callee, args, .. } => {
            ...
            for arg in args {
                match arg {
                    oxc_react_compiler_hir::PlaceOrSpread::Place(p) => {
                        resolve_identifier(p.identifier, identifiers, types, unifier);
                    }
                    oxc_react_compiler_hir::PlaceOrSpread::Spread(s) => {
        ...
        InstructionValue::JsxExpression {
            ...
            if let oxc_react_compiler_hir::JsxTag::Place(p) = tag {
                resolve_identifier(p.identifier, identifiers, types, unifier);
            }
            for attr in props {
                match attr {
