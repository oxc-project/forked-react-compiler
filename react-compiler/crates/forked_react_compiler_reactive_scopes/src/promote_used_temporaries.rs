...
fn collect_promotable_value(
    ...
    match value {
        ReactiveValue::Instruction(instr_value) => {
            // Visit operands
            for place in
                forked_react_compiler_hir::visitors::each_instruction_value_operand(instr_value, env)
            {
            ...
            match iv {
                ...
                | InstructionValue::Destructure { .. } => {
                    ...
                    if let InstructionValue::Destructure { lvalue, .. } = iv {
                        ...
                        {
                            for operand in
                                forked_react_compiler_hir::visitors::each_pattern_operand(&lvalue.pattern)
                            {
                    ...
                    // Visit operands
                    for place in
                        forked_react_compiler_hir::visitors::each_instruction_value_operand(iv, env)
                    {
...
fn promote_interposed_value(
    ...
    match value {
        ReactiveValue::Instruction(iv) => {
            for place in forked_react_compiler_hir::visitors::each_instruction_value_operand(iv, env) {
                promote_interposed_place(&place, state, inter_state, consts, env);
            }
        }
        ReactiveValue::SequenceExpression {
