...
use forked_react_compiler_hir::EvaluationOrder;
use forked_react_compiler_hir::FunctionId;
use forked_react_compiler_hir::IdentifierName;
...
use forked_react_compiler_hir::PrunedReactiveScopeBlock;
use forked_react_compiler_hir::ReactiveBlock;
use forked_react_compiler_hir::ReactiveFunction;
use forked_react_compiler_hir::ReactiveScopeBlock;
use forked_react_compiler_hir::ReactiveValue;
...
impl Scopes {
    ...
    fn visit_identifier(
        &mut self,
        identifier_id: forked_react_compiler_hir::IdentifierId,
        env: &Environment,
    ) {
...
impl ReactiveFunctionVisitor for Visitor<'_> {
    ...
    fn visit_scope(&self, scope: &ReactiveScopeBlock, state: &mut Scopes) {
        ...
        let decl_ids: Vec<forked_react_compiler_hir::IdentifierId> = scope_data
            .declarations
            .iter()
...
fn collect_globals_block(block: &ReactiveBlock, globals: &mut HashSet<String>, env: &Environment) {
    for stmt in block {
        match stmt {
            forked_react_compiler_hir::ReactiveStatement::Instruction(instr) => {
                collect_globals_value(&instr.value, globals, env);
            }
            forked_react_compiler_hir::ReactiveStatement::Scope(scope) => {
                collect_globals_block(&scope.instructions, globals, env);
            }
            forked_react_compiler_hir::ReactiveStatement::PrunedScope(scope) => {
                collect_globals_block(&scope.instructions, globals, env);
            }
            forked_react_compiler_hir::ReactiveStatement::Terminal(terminal) => {
...
fn collect_globals_terminal(
    stmt: &forked_react_compiler_hir::ReactiveTerminalStatement,
    globals: &mut HashSet<String>,
    env: &Environment,
) {
    match &stmt.terminal {
        forked_react_compiler_hir::ReactiveTerminal::Break { .. }
        | forked_react_compiler_hir::ReactiveTerminal::Continue { .. } => {}
        forked_react_compiler_hir::ReactiveTerminal::Return { .. }
        | forked_react_compiler_hir::ReactiveTerminal::Throw { .. } => {}
        forked_react_compiler_hir::ReactiveTerminal::For {
            init,
            test,
            update,
            loop_block,
        ...
        forked_react_compiler_hir::ReactiveTerminal::ForOf {
            init,
            test,
            loop_block,
            ..
        ...
        forked_react_compiler_hir::ReactiveTerminal::ForIn {
            init, loop_block, ..
        } => {
            collect_globals_value(init, globals, env);
            collect_globals_block(loop_block, globals, env);
        }
        forked_react_compiler_hir::ReactiveTerminal::DoWhile {
            loop_block, test, ..
        } => {
            collect_globals_block(loop_block, globals, env);
            collect_globals_value(test, globals, env);
        }
        forked_react_compiler_hir::ReactiveTerminal::While {
            test, loop_block, ..
        } => {
            collect_globals_value(test, globals, env);
            collect_globals_block(loop_block, globals, env);
        }
        forked_react_compiler_hir::ReactiveTerminal::If {
            consequent,
            alternate,
            ..
        } => {
        ...
        forked_react_compiler_hir::ReactiveTerminal::Switch { cases, .. } => {
            for case in cases {
        ...
        forked_react_compiler_hir::ReactiveTerminal::Label { block, .. } => {
            collect_globals_block(block, globals, env);
        }
        forked_react_compiler_hir::ReactiveTerminal::Try { block, handler, .. } => {
            collect_globals_block(block, globals, env);
