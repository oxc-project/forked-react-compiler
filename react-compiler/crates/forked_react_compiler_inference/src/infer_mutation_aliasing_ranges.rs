...
use forked_react_compiler_diagnostics::{CompilerDiagnostic, ErrorCategory};
use forked_react_compiler_hir::environment::Environment;
use forked_react_compiler_hir::type_config::{ValueKind, ValueReason};
...
pub fn infer_mutation_aliasing_ranges(
    ...
    for &block_id in &block_order {
        ...
        if let forked_react_compiler_hir::Terminal::Return { value, .. } = terminal {
        ...
        let terminal_effects = match terminal {
            forked_react_compiler_hir::Terminal::MaybeThrow { effects, .. }
            | forked_react_compiler_hir::Terminal::Return { effects, .. } => effects.clone(),
            _ => None,
        };
    ...
    for param in &mut func.params {
        let place = match param {
            forked_react_compiler_hir::ParamPattern::Place(p) => p,
            forked_react_compiler_hir::ParamPattern::Spread(s) => &mut s.place,
        };
    ...
    for &block_id in &block_order {
        ...
        match &mut block.terminal {
            forked_react_compiler_hir::Terminal::Return { value, .. } => {
    ...
    for param in &func.params {
        let place = match param {
            forked_react_compiler_hir::ParamPattern::Place(p) => p.clone(),
            forked_react_compiler_hir::ParamPattern::Spread(s) => s.place.clone(),
        };
        tracked.push(place);
    }
    for ctx in &func.context {
