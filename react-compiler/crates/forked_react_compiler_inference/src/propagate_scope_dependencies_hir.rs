...
use forked_react_compiler_hir::visitors::{ScopeBlockTraversal, ScopeBlockInfo};
...
struct MatchConsequentResult {
    ...
    property_load_loc: Option<forked_react_compiler_hir::SourceLocation>,
}
        ...
        for &instr_id in &block.instructions {
            ...
            match &instr.value {
                ...
                InstructionValue::JsxExpression { props, children, .. } => {
                    ...
                    for prop in props {
                        if let forked_react_compiler_hir::JsxAttribute::Attribute { place, .. } = prop {
        ...
        for &instr_id in &block.instructions {
            ...
            if env.enable_preserve_existing_memoization_guarantees {
                if let InstructionValue::StartMemoize { deps: Some(deps), .. } = &instr.value {
                    for dep in deps {
                        if let forked_react_compiler_hir::ManualMemoDependencyRoot::NamedLocal { value: val, .. } = &dep.root {
...
impl<'a> DependencyCollectionContext<'a> {
    ...
    fn visit_dependency(&mut self, dep: ReactiveScopeDependency, env: &mut Environment) {
                ...
                for &scope_id in &orig_scope_stack {
                    if !self.scope_stack.contains(&scope_id) {
                        ...
                        if !already_declared {
                            ...
                            let new_decl = forked_react_compiler_hir::ReactiveScopeDeclaration {
        ...
        let dep = if forked_react_compiler_hir::is_use_ref_type(
