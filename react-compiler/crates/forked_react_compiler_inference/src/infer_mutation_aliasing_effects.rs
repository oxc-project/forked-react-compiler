...
use forked_react_compiler_hir::AliasingEffect;
use forked_react_compiler_hir::AliasingSignature;
use forked_react_compiler_hir::BlockId;
use forked_react_compiler_hir::DeclarationId;
use forked_react_compiler_hir::Effect;
use forked_react_compiler_hir::FunctionId;
use forked_react_compiler_hir::HirFunction;
...
use forked_react_compiler_hir::MutationReason;
use forked_react_compiler_hir::ParamPattern;
use forked_react_compiler_hir::Place;
use forked_react_compiler_hir::PlaceOrSpread;
use forked_react_compiler_hir::PlaceOrSpreadOrHole;
use forked_react_compiler_hir::ReactFunctionType;
use forked_react_compiler_hir::SourceLocation;
use forked_react_compiler_hir::Type;
use forked_react_compiler_hir::environment::Environment;
use forked_react_compiler_hir::object_shape::BUILT_IN_ARRAY_ID;
use forked_react_compiler_hir::object_shape::BUILT_IN_MAP_ID;
use forked_react_compiler_hir::object_shape::BUILT_IN_SET_ID;
use forked_react_compiler_hir::object_shape::FunctionSignature;
use forked_react_compiler_hir::object_shape::HookKind;
use forked_react_compiler_hir::type_config::ValueKind;
use forked_react_compiler_hir::type_config::ValueReason;
use forked_react_compiler_hir::visitors;
...
impl InferenceState {
    ...
    fn mutate_with_loc(
        ...
        if forked_react_compiler_hir::is_ref_or_ref_value(ty) {
            ...
            match &instr.value {
                InstructionValue::Destructure { lvalue, value, .. } => {
                    ...
                    match &lvalue.pattern {
                        forked_react_compiler_hir::Pattern::Object(obj_pat) => {
                            for prop in &obj_pat.properties {
                                if let forked_react_compiler_hir::ObjectPropertyOrSpread::Spread(s) = prop
                                {
...
fn infer_block(
    ...
    let action = {
        ...
        match &block.terminal {
            forked_react_compiler_hir::Terminal::Try {
                handler,
            ...
            forked_react_compiler_hir::Terminal::MaybeThrow {
                handler: Some(handler_id),
                ..
            } => TerminalAction::MaybeThrow {
                handler_id: *handler_id,
            },
            forked_react_compiler_hir::Terminal::Return { .. } => TerminalAction::Return,
    ...
    match action {
        ...
        TerminalAction::MaybeThrow { handler_id } => {
            if let Some(handler_param) = context.catch_handlers.get(&handler_id).cloned() {
                if state.is_defined(handler_param.identifier) {
                    ...
                    if let forked_react_compiler_hir::Terminal::MaybeThrow {
        ...
        TerminalAction::Return => {
            if !context.is_function_expression {
                let block_mut = func.body.blocks.get_mut(&block_id).unwrap();
                if let forked_react_compiler_hir::Terminal::Return {
                    ref value,
                    effects: ref mut term_effects,
                    ..
                } = block_mut.terminal
...
fn apply_signature(
    ...
    state: &mut InferenceState,
    instr_idx: u32,
    instr: &forked_react_compiler_hir::Instruction,
    env: &mut Environment,
    func: &HirFunction,
                ...
                for effect in aliasing_effects {
                    ...
                    if value_abstract.kind == ValueKind::Frozen {
                        ...
                        let variable = match &ident.name {
                            Some(forked_react_compiler_hir::IdentifierName::Named(n)) => {
                                format!("`{}`", n)
                            }
                            _ => "value".to_string(),
                        };
                        ...
                        if is_mutate {
                            ...
                            {
                                diagnostic.details.push(forked_react_compiler_diagnostics::CompilerDiagnosticDetail::Hint {
    ...
    match effect {
        ...
        AliasingEffect::Apply {
            ...
            if let Some(sig) = signature {
                ...
                // Legacy signature
                let mut todo_errors: Vec<forked_react_compiler_diagnostics::CompilerErrorDetail> =
                    Vec::new();
        ...
        | AliasingEffect::MutateTransitiveConditionally { .. }) => {
                ...
                )
            {
                    ...
                    && context.hoisted_context_declarations.contains_key(&decl_id)
                {
                    ...
                    let variable = match &ident.name {
                        Some(forked_react_compiler_hir::IdentifierName::Named(n)) => format!("`{}`", n),
                        _ => "value".to_string(),
                    };
                    let mut diagnostic = CompilerDiagnostic::new(
                        ErrorCategory::Immutability,
    ...
    match value {
        InstructionValue::ArrayExpression { elements, .. } => {
            ...
            for element in elements {
                match element {
                    forked_react_compiler_hir::ArrayElement::Place(p) => {
                        effects.push(AliasingEffect::Capture {
                            from: p.clone(),
                            into: lvalue.clone(),
                        });
                    }
                    forked_react_compiler_hir::ArrayElement::Spread(s) => {
                        let ty = &env.types
                    ...
                    forked_react_compiler_hir::ArrayElement::Hole => {}
                }
            }
        }
        InstructionValue::ObjectExpression { properties, .. } => {
            ...
            for property in properties {
                match property {
                    forked_react_compiler_hir::ObjectPropertyOrSpread::Property(p) => {
                        effects.push(AliasingEffect::Capture {
                            from: p.place.clone(),
                            into: lvalue.clone(),
                        });
                    }
                    forked_react_compiler_hir::ObjectPropertyOrSpread::Spread(s) => {
                        effects.push(AliasingEffect::Capture {
                            from: s.place.clone(),
                            into: lvalue.clone(),
                        });
        ...
        | InstructionValue::ComputedLoad { object, .. } => {
            ...
            if forked_react_compiler_hir::is_primitive_type(ty) {
                effects.push(AliasingEffect::Create {
                    into: lvalue.clone(),
                    value: ValueKind::Primitive,
                    reason: ValueReason::Other,
        ...
        InstructionValue::PropertyStore {
            ...
            let mutation_reason: Option<MutationReason> = {
                ...
                if let forked_react_compiler_hir::PropertyLiteral::String(prop_name) = property {
        ...
        InstructionValue::JsxExpression {
            ...
            for prop in props {
                if let forked_react_compiler_hir::JsxAttribute::Attribute {
                    place: prop_place, ..
                } = prop
                {
                    ...
                    if let Type::Function { return_type, .. } = prop_ty {
                        if forked_react_compiler_hir::is_jsx_type(return_type)
...
fn compute_effects_for_legacy_signature(
    ...
    env: &Environment,
    function_values: &HashMap<ValueId, FunctionId>,
    todo_errors: &mut Vec<forked_react_compiler_diagnostics::CompilerErrorDetail>,
) -> Vec<AliasingEffect> {
        ...
        && are_arguments_immutable_and_non_mutating(state, args, env, function_values)
    {
        ...
        for arg in args {
            match arg {
                ...
                | PlaceOrSpreadOrHole::Spread(forked_react_compiler_hir::SpreadPattern { place }) => {
...
fn get_argument_effect(
    ...
    Effect,
    Option<forked_react_compiler_diagnostics::CompilerErrorDetail>,
) {
    if !is_spread {
        (sig_effect, None)
        ...
        let detail = if sig_effect == Effect::Freeze {
            Some(forked_react_compiler_diagnostics::CompilerErrorDetail {
...
fn compute_effects_for_aliasing_signature_config(
    env: &mut Environment,
    config: &forked_react_compiler_hir::type_config::AliasingSignatureConfig,
    lvalue: &Place,
    receiver: &Place,
    args: &[PlaceOrSpreadOrHole],
    context: &[Place],
    ...
    for eff_config in &config.effects {
        match eff_config {
            forked_react_compiler_hir::type_config::AliasingEffectConfig::Freeze { value, reason } => {
            ...
            forked_react_compiler_hir::type_config::AliasingEffectConfig::Create { into, value, reason } => {
            ...
            forked_react_compiler_hir::type_config::AliasingEffectConfig::CreateFrom { from, into } => {
            ...
            forked_react_compiler_hir::type_config::AliasingEffectConfig::Assign { from, into } => {
                let froms = substitutions.get(from).cloned().unwrap_or_default();
                let intos = substitutions.get(into).cloned().unwrap_or_default();
                for f in &froms {
                    for t in &intos {
            ...
            forked_react_compiler_hir::type_config::AliasingEffectConfig::Alias { from, into } => {
                let froms = substitutions.get(from).cloned().unwrap_or_default();
                let intos = substitutions.get(into).cloned().unwrap_or_default();
                for f in &froms {
                    for t in &intos {
            ...
            forked_react_compiler_hir::type_config::AliasingEffectConfig::Capture { from, into } => {
                let froms = substitutions.get(from).cloned().unwrap_or_default();
                let intos = substitutions.get(into).cloned().unwrap_or_default();
                for f in &froms {
                    for t in &intos {
            ...
            forked_react_compiler_hir::type_config::AliasingEffectConfig::ImmutableCapture { from, into } => {
                let froms = substitutions.get(from).cloned().unwrap_or_default();
                let intos = substitutions.get(into).cloned().unwrap_or_default();
                for f in &froms {
                    for t in &intos {
            ...
            forked_react_compiler_hir::type_config::AliasingEffectConfig::Impure { place } => {
            ...
            forked_react_compiler_hir::type_config::AliasingEffectConfig::Mutate { value } => {
                let values = substitutions.get(value).cloned().unwrap_or_default();
                for v in values {
                    effects.push(AliasingEffect::Mutate { value: v, reason: None });
                }
            }
            forked_react_compiler_hir::type_config::AliasingEffectConfig::MutateTransitiveConditionally { value } => {
                let values = substitutions.get(value).cloned().unwrap_or_default();
                for v in values {
                    effects.push(AliasingEffect::MutateTransitiveConditionally { value: v });
                }
            }
            forked_react_compiler_hir::type_config::AliasingEffectConfig::Apply { receiver: r, function: f, mutates_function, args: a, into: i } => {
                ...
                if let (Some(recv), Some(func), Some(into)) = (recv, func, into) {
                    ...
                    for arg in a {
                        match arg {
                            forked_react_compiler_hir::type_config::ApplyArgConfig::Hole { .. } => {
                                apply_args.push(PlaceOrSpreadOrHole::Hole);
                            }
                            forked_react_compiler_hir::type_config::ApplyArgConfig::Place(name) => {
                            ...
                            forked_react_compiler_hir::type_config::ApplyArgConfig::Spread { place: name, .. } => {
                                if let Some(places) = substitutions.get(name) {
                                    if let Some(p) = places.first() {
                                        apply_args.push(PlaceOrSpreadOrHole::Spread(forked_react_compiler_hir::SpreadPattern { place: p.clone() }));
                                    }
                                }
                            }
                        }
                                ...
                                if let Some(places) = substitutions.get(&sp.place.identifier) {
                                    if let Some(place) = places.first() {
                                        apply_args.push(PlaceOrSpreadOrHole::Spread(
                                            forked_react_compiler_hir::SpreadPattern {
                                                place: place.clone(),
                                            },
                                        ));
                                    }
...
fn is_ref_or_ref_value_for_id(env: &Environment, id: IdentifierId) -> bool {
    ...
    forked_react_compiler_hir::is_ref_or_ref_value(ty)
}
...
fn is_phi_with_jsx(ty: &Type) -> bool {
    if let Type::Phi { operands } = ty {
        operands
            .iter()
            .any(|op| forked_react_compiler_hir::is_jsx_type(op))
    } else {
        false
    }
}
...
fn terminal_successors(terminal: &forked_react_compiler_hir::Terminal) -> Vec<BlockId> {
    use forked_react_compiler_hir::Terminal;
    match terminal {
...
fn each_pattern_items(pattern: &forked_react_compiler_hir::Pattern) -> Vec<PatternItem<'_>> {
    let mut items = Vec::new();
    match pattern {
        forked_react_compiler_hir::Pattern::Array(arr) => {
            for el in &arr.items {
                match el {
                    forked_react_compiler_hir::ArrayPatternElement::Place(p) => {
                        items.push(PatternItem::Place(p))
                    }
                    forked_react_compiler_hir::ArrayPatternElement::Spread(s) => {
                        items.push(PatternItem::Spread(&s.place))
                    }
                    forked_react_compiler_hir::ArrayPatternElement::Hole => {}
                }
            }
        }
        forked_react_compiler_hir::Pattern::Object(obj) => {
