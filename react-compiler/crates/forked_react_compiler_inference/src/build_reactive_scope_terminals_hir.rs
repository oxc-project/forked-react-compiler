...
use forked_react_compiler_hir::visitors::each_instruction_lvalue_ids;
use forked_react_compiler_hir::visitors::each_instruction_operand_ids;
use forked_react_compiler_hir::visitors::each_terminal_operand_ids;
use forked_react_compiler_lowering::get_reverse_postordered_blocks;
use forked_react_compiler_lowering::mark_instruction_ids;
use forked_react_compiler_lowering::mark_predecessors;
...
fn fix_scope_and_identifier_ranges(func: &HirFunction, env: &mut Environment) {
    ...
    let original_scope_ranges: Vec<forked_react_compiler_hir::MutableRange> = env
        .scopes
        .iter()
        .map(|s| s.range.clone())
        .collect();
