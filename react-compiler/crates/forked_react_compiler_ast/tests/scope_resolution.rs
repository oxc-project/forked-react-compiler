...
fn scope_info_round_trip() {
        ...
        })
    {
        ...
        let scope_info: forked_react_compiler_ast::scope::ScopeInfo =
...
fn rename_identifiers(file: &mut forked_react_compiler_ast::File, si: &ScopeInfo) {
    visit_program(&mut file.program, si);
}

fn visit_program(prog: &mut forked_react_compiler_ast::Program, si: &ScopeInfo) {
...
fn scope_resolution_rename() {
        ...
        })
    {
        ...
        let mut file: forked_react_compiler_ast::File = match serde_json::from_str(&ast_json) {
            Ok(f) => f,
            Err(e) => {
