...
pub fn transform(
    ...
    {
        oxc_react_compiler_ast::SourceType::Script
    } else {
        oxc_react_compiler_ast::SourceType::Module
    };
    let file = convert_module_with_source_type(module, source_text, source_type);
    let scope_info = build_scope_info(module);
    let result =
        oxc_react_compiler::entrypoint::program::compile_program(file, scope_info, options);
    ...
    let conversion_result = program_json.and_then(|raw_json| {
        ...
        let file: oxc_react_compiler_ast::File = serde_json::from_value(value).ok()?;
