...
use forked_react_compiler_hir::visitors::{each_instruction_value_operand, each_terminal_operand};
            ...
            match &instr.value {
                InstructionValue::Destructure { value, lvalue, .. } => {
                    if inlined_state.contains_key(&env.identifiers[value.identifier.0 as usize].id) {
                        if let forked_react_compiler_hir::Pattern::Array(arr) = &lvalue.pattern {
                                ...
                                if let PlaceOrSpread::Place(arg) = &args[0] {
                                    let arg_type = &env.types
                                        [env.identifiers[arg.identifier.0 as usize].type_.0
                                            as usize];
                                    if forked_react_compiler_hir::is_primitive_type(arg_type)
                                        || forked_react_compiler_hir::is_plain_object_type(arg_type)
                                        || forked_react_compiler_hir::is_array_type(arg_type)
                                    {
                                        let lvalue_id = env.identifiers
                                            [instr.lvalue.identifier.0 as usize]
                                            .id;
        ...
        for &instr_id in &block.instructions {
            ...
            match &instr.value {
                ...
                InstructionValue::JsxExpression { tag, .. } => {
                    if let forked_react_compiler_hir::JsxTag::Builtin(builtin) = tag {
                        ...
                        if !builtin.name.contains('-') {
                                ...
                                &mut instr.value
                            {
                                props.retain(|prop| match prop {
                                    forked_react_compiler_hir::JsxAttribute::SpreadAttribute { .. } => {
                                        true
                                    }
                                    forked_react_compiler_hir::JsxAttribute::Attribute {
                                        name, ..
                                    } => {
                            ...
                            if !arr.items.is_empty() {
                                if let ArrayPatternElement::Place(first_place) = &arr.items[0] {
                                    ...
                                    let store = InstructionValue::StoreLocal {
                                        lvalue: forked_react_compiler_hir::LValue {
...
enum InlinedStateReplacement {
    ...
    LoadLocal {
        place: forked_react_compiler_hir::Place,
        loc: Option<forked_react_compiler_hir::SourceLocation>,
    },
    /// Replace with `CallExpression { callee, args: [arg] }` — used for useReducer(reducer, initialArg, init)
    CallExpression {
        callee: forked_react_compiler_hir::Place,
        arg: forked_react_compiler_hir::Place,
        loc: Option<forked_react_compiler_hir::SourceLocation>,
    },
}
