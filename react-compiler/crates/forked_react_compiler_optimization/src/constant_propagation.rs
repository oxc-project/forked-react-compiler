...
use forked_react_compiler_lowering::{
...
fn constant_propagation_impl(
    ...
    loop {
         ...
         */
        forked_react_compiler_ssa::eliminate_redundant_phi(func, env);
        ...
        InstructionValue::StartMemoize { deps, .. } => {
            if let Some(deps) = deps {
                ...
                let const_dep_indices: Vec<usize> = deps
                    ...
                    .enumerate()
                    .filter_map(|(i, dep)| {
                        if let forked_react_compiler_hir::ManualMemoDependencyRoot::NamedLocal {
                            value,
                            ..
                        } = &dep.root
                        {
