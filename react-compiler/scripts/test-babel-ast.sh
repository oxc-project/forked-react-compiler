...
FIXTURE_JSON_DIR="$TMPDIR" ~/.cargo/bin/cargo test -p forked_react_compiler_ast --test round_trip -- --nocapture

echo "Running scope resolution test..."
FIXTURE_JSON_DIR="$TMPDIR" ~/.cargo/bin/cargo test -p forked_react_compiler_ast --test scope_resolution -- --nocapture
