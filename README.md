...
(`react-compiler/Cargo.toml`) whose crates include `forked_react_compiler_oxc`, alongside the
reference TypeScript `babel-plugin-react-compiler` and its test fixtures.
...
crates can be published to crates.io, re-applied automatically on every `just sync`. It:

- renames every crate to **`forked_react_compiler_*`** (`react_compiler_oxc` →
  `forked_react_compiler_oxc`, etc.): directories, package names, path deps, and source;
